...
public class NurseQueryHandler {
    ...
    @QueryHandler
    NurseRestModel findNurseByUsernameAndPassword(FindNurseQueryByUsernameAndPassword query){

        Optional<NurseEntity> storedNurses = nurseRepository.findByUsernameAndPassword(query.getUsername(), query.getPassword());
        NurseRestModel nurseRestModel = new NurseRestModel();
        BeanUtils.copyProperties(storedNurses.get(), nurseRestModel);
