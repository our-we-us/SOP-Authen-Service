...
import com.example.nurseservice.query.FindNurseQueryByUsernameAndPassword;
...
public class NurseQueryController {
    ...
    @GetMapping(value = "/checklogin/{username}/{password}")
    public boolean getNurseByUsernameAndPassword(@PathVariable String username, @PathVariable String password){
        FindNurseQueryByUsernameAndPassword findNurseQueryByUsernameAndPassword = new FindNurseQueryByUsernameAndPassword();
        findNurseQueryByUsernameAndPassword.setUsername(username);
        findNurseQueryByUsernameAndPassword.setPassword(password);

        NurseRestModel nurse = queryGateway
                .query(findNurseQueryByUsernameAndPassword, ResponseTypes.instanceOf(NurseRestModel.class)).join();
        if(nurse != null){
            return true;
        }
        return false;
